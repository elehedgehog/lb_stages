<template>
  <div>
    ...
    <!-- 审核中 内容部分 -->
    <!-- <div class="check-ing" v-if="checkPop">
    ...
    <div class="check-success miss-header-bottom" align="center" v-if="checkSucessPop" >      <!--审核完成  内容部分-->
      <button @click="toBindCard()">绑卡确认</button>
    </div>
  ...
  export default {
      data(){
        return{
            ...
            checkPop: true, //审批中判断
            checkSucessPop: false, //审批通过绑卡中判断
            status: '',
            repayPrincipal: '',
            repayAmount: '',
            orderNo: '',
...
//        this.token = sessionStorage.OnCheckToken
      ...
      methods:{
        ...
        toBindCard(){  //跳转子页审批详情
          ...
          this.$router.push({ name: 'BindCard', params: {type: 'bindCardType' }, query: params })
        },
