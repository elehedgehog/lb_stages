<template>
  <div>
      ...
      <div class="selectBank-content" id="content">
        <div class="seletTitle">以下为您在钱宝绑定的借记卡信息,请选择：</div>
   ...
   export default {
      ...
      methods:{
        ...
        bindCardBtn(){     //绑定银行卡按钮 添加新银行卡
          ...
          this.$router.push({ name: 'NoSenseBandCard', params: type, query: params })
        },
        confirmBandCard(){     //确认按钮
          ...
          let paramsAll =  Object.assign(paramsTwo, this.$route.query)
          if(!bankSelected.bankBranch || !bankSelected.reservedMobile) {
            this.$router.push({ name: 'NoSenseBandCard',  query: Object.assign(paramsTwo, this.$route.query)})
          } else {
            // 绑卡接口 判断有感 无感
            this.getBindCard(paramsAll)
          }
        },
        getBindCard(paramsAll) {   //绑卡接口
